defmodule PtcLlmHttp.Scripts.DuplicationGateTest do
  ...
  setup do
    root =
      Path.join(
        System.tmp_dir!(),
        "ptc-llm-http-duplication-gate-#{System.unique_integer([:positive, :monotonic])}"
      )
  ...
  test "changed clone content is new duplication", context do
    bless(context, [clone(:type_i, 40, [{"lib/a.ex", 1, "old"}, {"lib/b.ex", 1, "old"}])])

    changed = clone(:type_i, 40, [{"lib/a.ex", 1, "new"}, {"lib/b.ex", 1, "new"}])

    assert {output, 1} = check(context, [changed])
    assert output =~ "NEW"
  end

  test "adding an occurrence is duplication growth", context do
  ...
  test "a smaller, less exact remnant with an unchanged occurrence passes", context do
    ...
    remnant = clone(:type_ii, 35, [{"lib/a.ex", 1, "same"}, {"lib/b.ex", 1, "changed"}])
