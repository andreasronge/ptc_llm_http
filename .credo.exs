    ...
    %{
      ...
      checks: %{
        enabled: [
          ...
          # Two recommended checks are omitted; see `disabled:`.
        ...
        disabled: [
          # False positives on NaN self-comparison (f == f filters out NaN).
          {Credo.Check.Warning.OperationOnSameValues, []},

          # ExSlop checks omitted from the recommended set.
          #
          # Fires on `length/1` against a literal in any context. In this
          # repository every hit is an ExUnit assertion or a guard on a short,
          # bounded list; the suggested `Enum.count_until/2` rewrite is less
          # readable at those sizes.
          {ExSlop.Check.Refactor.LengthComparison, []},
          # Cannot distinguish a compile-time `@attr Path.expand(..., __DIR__)`
          # paired with `@external_resource` -- the form wire fixtures use --
          # from a runtime priv lookup, which is what it means to flag.
